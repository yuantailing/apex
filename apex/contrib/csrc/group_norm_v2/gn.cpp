...
unsigned *get_persistent_barrier(cudaStream_t stream, int64_t size) {
    static std::mutex mut;
    static std::map<std::pair<cudaStream_t, int64_t>, torch::Tensor> barriers;
    if (size > 0) {
        size = group_norm_v2::round_up_pow2(size);
    }
    std::lock_guard<std::mutex> lock(mut);
    auto it = barriers.find({stream, size});
    if (it == barriers.end()) {
        it = barriers.insert({{stream, size}, torch::zeros({size}, torch::TensorOptions().dtype(torch::kUInt32).device(torch::kCUDA))}).first;
    }
    return it->second.data_ptr<unsigned>();
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("gn", &group_norm_v2::gn, py::arg("x"), py::arg("w"), py::arg("b"), py::arg("eps"), py::arg("silu"), py::arg("num_groups"), py::arg("mean_var_out") = py::none(), py::arg("sm_margin") = 0, "", py::call_guard<py::gil_scoped_release>());
    m.def("gn_bwd", &group_norm_v2::gn_bwd, py::arg("grad_output"), py::arg("x"), py::arg("w"), py::arg("b"), py::arg("mean_var"), py::arg("eps"), py::arg("silu"), py::arg("num_groups"), py::arg("sm_margin") = 0, "", py::call_guard<py::gil_scoped_release>());
}
